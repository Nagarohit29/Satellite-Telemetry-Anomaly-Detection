...
import { X, Check, Edit2, Save, Trash2, Key, Loader2, RefreshCw } from 'lucide-react';
...
const SettingsModal = ({ isOpen, onClose, selectedModel, setSelectedModel }) => {
  ...
  const handleSaveKey = async (provider) => {
    ...
    if (provider !== 'ollama' && key.length < 20) {
  ...
  const handleDeleteKey = async (provider) => {
    if (!confirm(`Remove the API key for ${provider}?`)) return;
    try {
      setIsSaving(true);
      setSaveStatus(null);
      ...
      if (selectedModel === provider) setSelectedModel('');
      await fetchModels();
    } catch (err) {
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          {!selectedModel && !loading && (
            <div style={{
              ...
              <span><strong>Auto Fallback</strong> — System will try all available models automatically. Select one to override.</span>
            </div>
          )}
                  ...
                  >
                    ...
                    <div className="model-info">
                      ...
                      <div className="model-status">
                        {model.available 
                          ? (selectedModel === model.id ? 'Currently Active' : 'Available — Click to select') 
                          : (model.id === 'ollama' ? 'Server not running' : 'Missing API Key')}
                      </div>
                    </div>
                    <button 
                      className="btn"
